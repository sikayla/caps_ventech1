...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($username) || empty($password)) {
        ...
        $sql = "SELECT id, username, firstname, lastname, client_address, password FROM client WHERE username = ? OR email = ?";
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            $_SESSION['client_address'] = $user['client_address']; // ✅ Store Client Address

            header("Location: client_greet.php");
            exit();
        } else {
...
<body class="bg-gray-100">
    ...
    <div class="bg-white shadow-lg rounded-lg p-8 flex flex-col md:flex-row space-y-6 md:space-y-0 md:space-x-6 w-full max-w-4xl mx-auto mt-10">
        ...
        <div class="flex flex-col items-center w-full md:w-1/2">
            ...
            <form class="w-full max-w-sm" action="client_signin.php" method="POST">
                <div class="mb-4">
            ...
            <div class="flex justify-between w-full mt-4">
                <a class="text-red-500 hover:text-red-800" href="client_signup.php">Register here</a>
